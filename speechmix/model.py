...
class SpeechMixEED(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        remove_layers = (
            int(
                len(self.encoder_model.model.encoder.layers) * share_layer_ratio
            )
        ...
        self.weights_sum = nn.Parameter(
            torch.zeros(self.num_speech_encoder_layers)
        ).to(self.device)
    ...
    def forward(
        ...
        if self.weighted_sum:
            ...
            weighted_feature = (
                norm_weights.unsqueeze(-1) * stacked_feature
            ).sum(dim=0)
        ...
        for s_i, s in enumerate(decoder_stack):
            for l_i, l in enumerate(s):
                l.register_forward_hook(
                    lambda m, i, o: (
                        self.adapters[s_i * len(s) + l_i](o[0]),
                        o[1:],
                    )
                )
